...
@section('content')
    ...
    <div class="col-md-12">
        <div class="col-md-6">
            <div class="ls-box">
                <b class="ls-title-5"><span style="color: red;"><?php echo DB::table('vehiclerequests')
                    ->where('statussolicitacao', 'PENDENTE')
                    ->count(); ?></span> - Solicitações pendentes</b>
                <hr>
                    ...
                    <tbody>
                        @foreach ($vehiclerequests as $vehiclerequest)
                            <tr data-index="<?php echo $vehiclerequest->id; ?>">
                                ...
                                <td>
                                    @inject('adress', '\App\Adress')
                                    @foreach ($adress->getAdresses() as $adresses)
                                        @if ($vehiclerequest->origem === $adresses->slug_adress)
                                            <?php
                                            $origem = substr($adresses->acronym . ' - ' . $adresses->slug_adress, 0, strrpos(substr($vehiclerequest->origem, 0, 15), ' ')) . '..';
                                            ?>
                                        @elseif ($vehiclerequest->origem != $adresses->slug_adress)
                                            <?php
                                            $origem = substr($vehiclerequest->origem, 0, strrpos(substr($vehiclerequest->origem, 0, 15), ' ')) . '..';
                                            ?>
                                        @endif
                                        @if ($vehiclerequest->destino === $adresses->slug_adress)
                                            <?php
                                            $destino = substr($adresses->acronym . ' - ' . $adresses->slug_adress, 0, strrpos(substr($vehiclerequest->destino, 0, 35), ' ')) . '..';
                                            ?>
                                        @elseif ($vehiclerequest->destino != $adresses->slug_adress)
                                            <?php
                                            $destino = substr($vehiclerequest->destino, 0, strrpos(substr($vehiclerequest->destino, 0, 15), ' ')) . '..';
                                            ?>
                                        @endif
                                    @endforeach
                                ...
                                <td><input class="limited" type="checkbox"
                                        onclick="cloneOrRemove(this,'<?php echo $vehiclerequest->id; ?>')"></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
                ...
                <b class="ls-title-5">Roteiro
                    <div align="right" style="font-weight: bold; position: relative; top: -25px; margin-bottom: -25px;">
                        <a id="openModal" onclick="parseInfo()" data-ls-module="modal" data-target="#modalLarge"
                            class="ls-ico-user-add ls-btn-primary"> Definir</a href="#">
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <div class="ls-custom-select">
                                            <select id="" name="driver" class="ls-select form-control" required
                                                oninvalid="this.setCustomValidity('Selecione um motorista!')"
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <div class="ls-custom-select">
                                            <select name="vehicle" class="ls-select" required
                                                ...
                                                onchange="try{setCustomValidity('')}catch(e){}">
                                                <?php $vehicles = DB::table('vehicles')
                                                    ->where('id', '!=', '1')
                                                    ->where('id', '!=', '2')
                                                    ->get(); ?>
